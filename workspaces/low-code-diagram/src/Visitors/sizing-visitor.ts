...
function updateSquareViewState(node: any, viewState: any) {
  if (node.children !== undefined && node.children.length > 0) {
    let height = COMPONENT_GAP;
    let width = 0;
...
export class SizingVisitor implements Visitor {
  endVisitMediator?(node: Mediator) {
    if (node.viewState) {
      ...
      if (node.children !== undefined && node.children.length > 0) {
        ...
        node.children.forEach((child: any) => {
          const childVS: MediatorViewState =
            child.viewState as MediatorViewState;
          if (childVS !== undefined) {
            height += childVS.bBox.h + COMPONENT_GAP;
          }
        });
  ...
  endVisitForEach?(node: ForEach) {
    if (node.viewState) {
      ...
      updateSquareViewState(node, viewState);
    }
  }
