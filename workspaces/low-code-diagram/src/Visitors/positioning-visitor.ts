...
export class PositioningVisitor implements Visitor {
  beginVisitMediator(el: Mediator) {
    if (el.viewState) {
      ...
      el.children.forEach((child, index) => {
        ...
        childVS.bBox.x =
          viewState.bBox.x + viewState.bBox.w / 2 - childVS.bBox.w / 2;
  ...
  beginVisitInSequence?(node: InSequence) {
    if (node.viewState) {
      const viewState: InSequenceViewState =
        node.viewState as InSequenceViewState;
      ...
      node.children.forEach((child, index) => {
        ...
        childVS.bBox.y =
          viewState.bBox.y + viewState.bBox.h / 2 - childVS.bBox.h / 2;
  ...
  beginVisitOutSequence?(node: OutSequence) {
    if (node.viewState) {
      const viewState: OutSequenceViewState =
        node.viewState as OutSequenceViewState;
  ...
  beginVisitPropertyGroup?(node: PropertyGroup) {
    if (node.viewState) {
      const viewState: PropertyGroupViewState =
        node.viewState as PropertyGroupViewState;
      viewState.bBox.cx = viewState.bBox.x + viewState.bBox.r;
      viewState.bBox.cy = viewState.bBox.y + viewState.bBox.r;
    }
  }
  ...
  beginVisitTransaction?(node: Transaction) {
    if (node.viewState) {
      const viewState: TransactionViewState =
        node.viewState as TransactionViewState;
      viewState.bBox.cx = viewState.bBox.x + viewState.bBox.r;
      viewState.bBox.cy = viewState.bBox.y + viewState.bBox.r;
    }
  }
  ...
  beginVisitURLrewrite?(node: URLrewrite) {
    if (node.viewState) {
      const viewState: URLrewriteViewState =
        node.viewState as URLrewriteViewState;
      viewState.bBox.cx = viewState.bBox.x + viewState.bBox.r;
      viewState.bBox.cy = viewState.bBox.y + viewState.bBox.r;
    }
  }
  ...
  beginVisitCallTemplate?(node: CallTemplate) {
    if (node.viewState) {
      const viewState: CallTemplateViewState =
        node.viewState as CallTemplateViewState;
      viewState.bBox.cx = viewState.bBox.x + viewState.bBox.r;
      viewState.bBox.cy = viewState.bBox.y + viewState.bBox.r;
    }
  }
  ...
  beginVisitConditionalRouter?(node: ConditionalRouter) {
    if (node.viewState) {
      const viewState: ConditionalRouterViewState =
        node.viewState as ConditionalRouterViewState;
      viewState.bBox.cx = viewState.bBox.x + viewState.bBox.r;
      viewState.bBox.cy = viewState.bBox.y + viewState.bBox.r;
    }
  }
  ...
  beginVisitDataMapper?(node: DataMapper) {
    if (node.viewState) {
      const viewState: DataMapperViewState =
        node.viewState as DataMapperViewState;
      viewState.bBox.cx = viewState.bBox.x + viewState.bBox.r;
      viewState.bBox.cy = viewState.bBox.y + viewState.bBox.r;
    }
  }
  ...
  beginVisitDataServiceCall?(node: DataServiceCall) {
    if (node.viewState) {
      const viewState: DataServiceCallViewState =
        node.viewState as DataServiceCallViewState;
      viewState.bBox.cx = viewState.bBox.x + viewState.bBox.r;
      viewState.bBox.cy = viewState.bBox.y + viewState.bBox.r;
    }
  }
  ...
  beginVisitJsonTransform?(node: JsonTransform) {
    if (node.viewState) {
      const viewState: JsonTransformViewState =
        node.viewState as JsonTransformViewState;
      viewState.bBox.cx = viewState.bBox.x + viewState.bBox.r;
      viewState.bBox.cy = viewState.bBox.y + viewState.bBox.r;
    }
  }
  ...
  beginVisitPayloadFactory?(node: PayloadFactory) {
    if (node.viewState) {
      const viewState: PayloadFactoryViewState =
        node.viewState as PayloadFactoryViewState;
      viewState.bBox.cx = viewState.bBox.x + viewState.bBox.r;
      viewState.bBox.cy = viewState.bBox.y + viewState.bBox.r;
    }
  }
  ...
  beginVisitAggregate?(node: Aggregate) {
    if (node.viewState) {
      const viewState: AggregateViewState =
        node.viewState as AggregateViewState;
  ...
  beginVisitEntitlement?(node: Entitlement) {
    if (node.viewState) {
      const viewState: EntitlementViewState =
        node.viewState as EntitlementViewState;
