...
export default class DMCFile {
	...
	public static fileCreation(linkData: [],name : string) {
		...
		if (workspaceFolder) {
			...
			const inputFilePath = fs.readdirSync(workspaceFolderPath).find((file: string) => (file.startsWith(name) && file.endsWith('_InputSchema.json')));
			const outputFilePath = fs.readdirSync(workspaceFolderPath).find((file: string) => (file.startsWith(name) && file.endsWith('_OutputSchema.json')));
			let inputData;
			let outputData;
