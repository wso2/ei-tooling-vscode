...
//File to manage Input and Output nodes when files are uploaded
const NodeManager = () => {
  ...
  React.useEffect(() => {
    ...
    if (schemaInput) {
      InputDataMapper = new DataMapperNodeModel(schemaInput.properties, {
        name: "Input",
      });
  ...
  React.useEffect(() => {
    const newNodes: CustomNodeModel[] = [];
    if (schemaOutput) {
      OutputDataMapper = new DataMapperNodeModel(schemaOutput.properties, {
        name: "Output",
      });
