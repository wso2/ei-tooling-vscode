...
export const DataMapperPortWidget: React.FC<DataMapperPortWidgetProps> = ({
  port,
  engine,
}) => {
  const classes = portStyles();
  const checkedIcon = (
    <RadioButtonChecked color="disabled" sx={{ fontSize: "16px" }} />
  );
  const uncheckedIcon = (
    <RadioButtonUnchecked color="disabled" sx={{ fontSize: "16px" }} />
  );
  const [portState, setPortState] = React.useState<PortState>(
    PortState.Unselected
  );
  ...
  React.useEffect(() => {
    port.registerListener({
      selectionChanged: () => {
        ...
        setPortState(
          isSelected ? PortState.PortSelected : PortState.Unselected
        );
      },
    });
  }, [port]);
  ...
  return (
    <PortWidget
      ...
      style={{
        color:
          portState === PortState.PortSelected
            ? "#93a2b5"
            : hasLinks
            ? "#96C0CE"
            : "#FEF6EB",
      ...
      <div className={classes.port}>
        {port.portType === "OUT" ? (
          <div className={classes.portLabel}>
            {port.getName()}{" "}
            {portState === PortState.PortSelected ? checkedIcon : uncheckedIcon}
          </div>
        ) : (
