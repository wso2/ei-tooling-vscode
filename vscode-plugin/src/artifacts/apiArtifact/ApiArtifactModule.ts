...
import {Uri, window, workspace, WorkspaceEdit, WorkspaceFolder, Position} from "vscode";
import * as fse from "fs-extra";
import * as path from 'path';

export namespace APIArtifactModule {
    ...
    export async function createTemplate(targetFolder: string, sourceFile: string, destinationFile: string) {
        ...
        if(workspace.workspaceFolders) {
            ...
            if(workspaces === 0) {
                ...
                findSynapseDirectoryNew(path1).then(result => {
                    if(!result) {
                        ...
                        findSynapseDirectoryNew(path2).then(result => {
                            if(!result) {
                                window.showErrorMessage("Make sure that you have opened a Synapse Project");
                            }else {
    ...
    async function findSynapseDirectoryNew(path: string): Promise<boolean> {
    ...
    async function createTargetArtifact(sourceFile: string, uri: Uri) {
        ...
        let sourcePath = path.join(directoryName, '..', '..', '..', 'templates', sourceFile+'.xml');
    ...
    async function open(uri: Uri): Promise<any> {
